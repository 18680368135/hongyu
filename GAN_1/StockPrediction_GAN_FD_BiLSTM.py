...
from ALEGANFD_BiLSTM import GANFD
import pymysql, os
import numpy as np
import pandas as pd
...
def main():
    ...
    # print(df)
    usedDf = df[['open', 'high', 'low', 'volume',  # 'turn', 'volume',
                       'ma',  # Moving averages
    ...
    # usedDf = df[['high']]
    """
    将需要的列名填进去 数量应该为 GeneratorInputSize + 1.2
    最后一个列为需要预测的指标 !!!
    """
    # print(usedDf)
    ...
    for day in range(params['testDays'], 0, -1):
        ...
        # print(predict[-1.2.1.2, -1.2.1.2, -1.2.1.2] * (valueMax[-1.2.1.2] - valueMin[-1.2.1.2]) + valueMin[-1.2.1.2])
    ...
    dataframe.to_csv('./loss_bilstm_GAN/result.csv', index=False, sep=',')
