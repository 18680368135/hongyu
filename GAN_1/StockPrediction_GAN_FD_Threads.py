...
from ALEGANFD_v3_process import GANFD
import pymysql, os
import numpy as np
import pandas as pd
...
def dataprocessandpredict(q, dataProcessor, day):
    ...
    predict_list = []
    for i in range(5):
        predict, mark = model.trainAndPredict(normalizatedData, dataProcessor, day, os.getppid(), os.getpid())
        while mark:
            predict, mark = model.trainAndPredict(normalizatedData, dataProcessor, day, os.getppid(), os.getpid())
...
def readqueue(q):
    ...
    df.to_csv('./loss4/result.csv', index=False, sep=',')
