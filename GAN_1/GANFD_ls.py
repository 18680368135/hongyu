...
class GANFD():
    ...
    def buildModel(self):
        ...
        self.gLoss = tf.reduce_mean(tf.square(self.dLogitsFake - 1.))  # + self.squareLoss + self.directLoss
    ...
    def trainAndPredict(self, data, dataProcessor, day, parprocess, chilprocess):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            with self.graph.as_default():
                ...
                if not os.path.isdir('./loss_ls/'):
                    os.mkdir('./loss_ls/')

                if not os.path.isdir('./loss_ls/%d/' % day):
                    os.mkdir('./loss_ls/%d/' % day)

                dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
                dataframe.to_csv('./loss_ls/%d/realFakeLoss.csv' % day, index=False, sep=',')

                dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
                dataframe.to_csv('./loss_ls/%d/GANLoss.csv' % day, index=False, sep=',')
                dataframe = pd.DataFrame({'D': trainHist['squareLoss']})
                dataframe.to_csv('./loss_ls/%d/Loss.csv' % day, index=False, sep=',')
