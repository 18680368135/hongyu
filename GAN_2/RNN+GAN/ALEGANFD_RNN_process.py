...
class GANFD():
    ...
    def generator(self, generatorInput):
        rnn = RNN(rnn_unit = self.hiddenUnit, input_size = self.genInputSize,
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as self.sess:
                ...
                for st_index in range(step_index_len):
                    ...
                    if st_index == 0:
                        ...
                        for can_i in range(0, self.ncandi):
                            ...
                            if frScore[0] < 5 * 1e-4 and frScore[1] < 5* 1e-4 and frScore[2] < 5*1e-4:
                                return _, True
            ...
            if not os.path.isdir('./loss_rnn_GAN/'):
                os.mkdir('./loss_rnn_GAN/')

            if not os.path.isdir('./loss_rnn_GAN/%d/' % day):
                os.mkdir('./loss_rnn_GAN/%d/' % day)

            dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
            dataframe.to_csv('./loss_rnn_GAN/%d/realFakeLoss.csv' % day, index=False, sep=',')

            dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
            dataframe.to_csv('./loss_rnn_GAN/%d/GANLoss.csv' % day, index=False, sep=',')
