...
class GANFD():
    ...
    def buildModel(self):
        ...
        self.gLoss_ls = tf.reduce_mean(tf.squared_difference(self.dLogitsFake,
                                                             tf.ones_like(self.dLogitsFake)))   #  + self.squareLoss + self.directLoss
                                ...
                                if self.loss[type_i] == 'trickLogD':
                                    ...
                                    _, gLossVal = self.sess.run([self.gOptim_ls, self.gLoss_ls],
                                                                feed_dict={self.genInput: genIn[type_i],
                                                                           self.realData: real[type_i]})
                                # 计算适应度函数值
                                fr_score, fd_score, dlossvalue, pFakeVal = self.sess.run(
                                                  [self.fakeOut, self.Fd_score, self.dLoss, self.pGen],
                                                  feed_dict={
