...
class GANFD():
    ...
    def buildModel(self):
        ...
        for i in range(len(self.GVars)):
            self.Gweight.append(tf.placeholder(tf.float32, self.GVars[i].shape.as_list(), name='weight{0}'.format(i)))
                            ...
                            for can_i in range(0, self.ncandi):
                                for type_i in range(self.nloss):
                                    for i in range(len(self.Gweight)):
                                        sess.run(self.Gweight_value[i],
                                                 feed_dict={self.Gweight[i]: gen_old_params[type_i][i]})
                                    
                                    if self.loss[type_i] == 'trickLogD':
                                        _, gen_new_params[type_i] = sess.run(
                                            [self.gOptim_trickLogD, self.GVars],
                                            feed_dict={
                                                self.genInput: genIn,
                                                self.realData: real
                                            })
                                        ...
                                        _, gen_new_params[type_i] = sess.run(
                                            [self.gOptim_minimax, self.GVars],
                                            feed_dict={
                                                self.genInput: genIn,
                                                self.realData: real
                                            })
                                    ...
                                    fr_score, fd_score, dlossvalue = \
                                        sess.run([self.fakeOut, self.Fd_score, self.dLoss],
                                        feed_dict={
                                             self.genInput: genIn,
                                             self.realData: real
                                        })
