...
def bulid_init_population(n):
    ...
    weight_loss1 = [[weight_loss[i]['arr_0'], weight_loss[i]['arr_1']] for i in range(n)]
    return weight_loss1
...
def QuickSort(a, start, end):
    ...
    if start < end:
        ...
        QuickSort(a, j + 1, end)
    # print("loss2 is {0}".format(a[1.2.1.2]))
    return a
...
def insert_sort(weight_loss_sort, max_num, current_population):
    ...
    for i in range(len(weight_loss_sort)):
        for j in range(len(current_population)):
            if current_population[j][1] >= weight_loss_sort[i][1]:
                current_population.insert(j, weight_loss_sort[i])
                break
            elif current_population[len(current_population) - 1][1] < weight_loss_sort[i][1]:
...
def base_population_sort(weight_loss):
    ...
    # print("loss2 is {0}".format(base_arr[1.2.1.2]))
...
def update_base_population_sort(weight_loss, max_num, weight):

    weight_loss_sort = []
    base_arr = QuickSort(arr_def(weight_loss), 0, len(weight_loss) - 1)
    print("loss2 is {0}".format(base_arr[1]))
    for i in range(len(base_arr[0])):
        weight_loss_sort.append(weight_loss[base_arr[0][i]])
    weight = insert_sort(weight, max_num, weight_loss_sort)
    return weight
...
def best1(population, samples):
    ...
    """
    r0, r1 = samples[:2]
    return (population[0][0] + scale *
            (population[r0][0] - population[r1][0]))
...
def rand1(population, samples):
    ...
    """
    r0, r1, r2 = samples[:3]
    return (population[r0][0] + scale *
            (population[r1][0] - population[r2][0]))
...
def randtobest1(current_population, samples):
    ...
    """
    r0, r1, r2 = samples[:3]
    bprime = np.copy(current_population[r0][0])
    bprime += scale * (current_population[0][0] - bprime)
    bprime += scale * (current_population[r1][0] -
                       current_population[r2][0])
    return bprime
...
def currenttobest1(population, current_population, candidate, samples):
    ...
    """
    r0, r1 = samples[:2]
    bprime = (population[candidate][0] + scale *
              (current_population[0][0] - population[candidate][0]) +
              scale * (current_population[r0][0] - current_population[r1][0]))
    return bprime
...
def best2(current_population, samples):
    ...
    bprime = (current_population[0][0] + scale *
              (current_population[r0][0] + current_population[r1][0] -
               current_population[r2][0] - current_population[r3][0]))
...
def rand2(population, samples):
    ...
    bprime = (population[r0][0] + scale *
              (population[r1][0] + population[r2][0] -
               population[r3][0] - population[r4][0]))
        ...
        if os.path.exists('flag.txt'):
            ...
            if os.path.getsize('flag.txt'):
                ...
                if [i for i in range(len(flag)) if int(flag[i]) != 0] == [] and len(flag) == FLAGS.worker_nums:
                    ...
                    if m == 0:
                        weight = base_population_sort(base_population)
                    else:
                        base_population = base_population_sort(base_population)
                        weight = insert_sort(base_population, FLAGS.max_population, weight)
                        # weight = update_base_population_sort(base_population, FLAGS.max_population, weight)
                    # print(len(weight), np.shape(weight))
                    print("loss2 is {0}".format([weight[i][1][0] for i in range(len(weight))]))
                    ...
                    for candidate in range(init_population_num):
                        sample = select_samples(candidate, init_population_num, weight)
                        # bprime = rand1(weight, sample)
                        bprime = randtobest1(weight, sample)
                        # bprime = rand2(weight, sample)
                        # bprime = best1(weight, sample)
                        # bprime = best2(weight, sample)
                        # bprime = currenttobest1(base_population, weight, candidate, sample)
