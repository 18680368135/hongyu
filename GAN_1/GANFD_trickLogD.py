    ...
    def buildModel(self):
        ...
        self.gLoss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                ...
                labels=tf.ones_like(self.dLogitsFake)))  # + self.squareLoss + self.directLoss
    ...
    def trainAndPredict(self, data, dataProcessor, day, parprocess, chilprocess):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            with self.graph.as_default():
                ...
                if not os.path.isdir('./loss_trickLogD/'):
                    os.mkdir('./loss_trickLogD/')

                if not os.path.isdir('./loss_trickLogD/%d/' % day):
                    os.mkdir('./loss_trickLogD/%d/' % day)

                dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
                dataframe.to_csv('./loss_trickLogD/%d/realFakeLoss.csv' % day, index=False, sep=',')

                dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
                dataframe.to_csv('./loss_trickLogD/%d/GANLoss.csv' % day, index=False, sep=',')
                dataframe = pd.DataFrame({'D': trainHist['squareLoss']})
                dataframe.to_csv('./loss_trickLogD/%d/Loss.csv' % day, index=False, sep=',')
