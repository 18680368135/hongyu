        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epochs):
                ...
                for train_input, train_label in data_generator(self.batch_size, train_data):
                    ...
                    _, _, _, _, cost, precision, recall, global_step = sess.run(
                        [self.optimizer, self.prediction_update_op, self.recall_update_op, self.accuracy_update_op,
                         self.loss, self.precision, self.recall, self.global_step],
                        feed_dict=feed_dict)
                    print("Epoch: {0}, global_step: {1}, training loss: {2}, "
                          "training recall: {3}, training precision: {4}".format(
