...
tf.app.flags.DEFINE_integer('max_population', 100, 'the max num of the population')
...
def randtobest1(current_population, samples):
    ...
    """
    r0, r1, r2 = samples[:3]
    bprime = np.copy(current_population[r0])
    bprime += scale * (current_population[0] - bprime)
    bprime += scale * (current_population[r1] -
                       current_population[r2])
    return bprime
...
def _best2(samples, current_population):
    """
    best2bin, best2exp
    """
    r0, r1, r2, r3 = samples[:4]
    bprime = (current_population[0] + scale *
              (current_population[r0] + current_population[r1] -
               current_population[r2] - current_population[r3]))

    return bprime


def _rand2(population, samples):
    """
    rand2bin, rand2exp
    """
    r0, r1, r2, r3, r4 = samples
    bprime = (population[r0] + scale *
              (population[r1] + population[r2] -
...
def select_samples(number_samples, current_population):
    """
    obtain random integers from range(len(current_population)),
    without replacement.  You can't have the original candidate either.
    """
    idex = list(range(len(current_population)))
        ...
        if os.path.exists('flag.txt'):
            ...
            if os.path.getsize('flag.txt'):
                ...
                if flag.size == 1:
                    print("flag 文件中只有一个值")
                    continue
                print("flag 文件中数字个数不止一个")
                if [i for i in range(len(flag)) if flag[i] != 0] == [] and len(flag) == FLAGS.worker_nums:
                    ...
                    for candidate in range(init_population_num):
                        sample = select_samples(init_population_num, current_population)
