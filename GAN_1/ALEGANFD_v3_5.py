...
class GANFD():
    ...
    def generator(self, generatorInput):
        lstm = BiLSTM(rnn_unit = self.hiddenUnit, input_size = self.genInputSize,
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as self.sess:
            ...
            if not os.path.isdir('./loss3_4/'):
                os.mkdir('./loss3_4/')

            if not os.path.isdir('./loss3_4/%d/' % day):
                os.mkdir('./loss3_4/%d/' % day)

            dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
            dataframe.to_csv('./loss3_4/%d/realFakeLoss.csv' % day, index=False, sep=',')

            dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
            dataframe.to_csv('./loss3_4/%d/GANLoss.csv' % day, index=False, sep=',')
