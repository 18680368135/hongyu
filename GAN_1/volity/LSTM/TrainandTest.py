...
class TrainandTest:
    ...
    def getdataset(self,original_data,input_num,test_day,start ,end, middle,train):
        X_train,X_test,Y_train,Y_test,reallabel,delta,smoothed_data_pre,ss_y=Movingdata_Pytorch3_26.get_all_data(original_data,input_num,test_day,start ,end, middle,train)  
        return X_train,X_test,Y_train,Y_test,reallabel,delta,smoothed_data_pre,ss_y
  
    

    def returndata(self,inputs,Y_test,delta,smoothed_data_pre,reallabel,ss_y):
        alpha=0.8    
        normal_output=ss_y.inverse_transform(inputs)
        smoothed_output=(normal_output-(1-alpha)*smoothed_data_pre)/alpha
        processed_output=smoothed_output*delta+delta
        return processed_output,reallabel
    ...
    def train_evaluate(self,epoch,model,original_data,batch_size,lr,input_num,test_day,train_start,test_end,train_end,cuda,optims,clip,log_interval):          
        ...
        X_train,X_test,Y_train,Y_test,reallabel,delta,smoothed_data_pre,ss_y=self.getdataset(original_data,                                  
...
#-------------------------------------------
            for i in range(0, X_train.size()[0], batch_size):
                ...
                if batch_idx % log_interval == 0:
                    ...
                    print('Train Epoch: {:2d} [{:6d}/{:6d} ({:.0f}%)]\tLearning rate: {:.5f}\tLoss: {:.6f}'.format(
        ...
        predict,reallabel=self.returndata(predict_output,Y_test,delta,smoothed_data_pre,reallabel,ss_y)   
