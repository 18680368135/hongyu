...
class RNN(object):
    ...
    def rnn(self, X):
        ...
        cell = tf.nn.rnn_cell.BasicRNNCell(self.rnn_unit)
        # [cell]*2为2层LSTM
        mrnn_cell = tf.contrib.rnn.MultiRNNCell([cell] * 1, state_is_tuple=True)
        init_state = mrnn_cell.zero_state(batch_size, dtype=tf.float32)

        output_rnn, final_states = tf.nn.dynamic_rnn(mrnn_cell, X, initial_state=init_state, dtype=tf.float32)
        # print(output_rnn)
