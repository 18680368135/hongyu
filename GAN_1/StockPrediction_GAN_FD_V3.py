...
def main():
    ...
    for day in range(params['testDays'], 0, -1):
        ...
        # print(predict.shape)
        # print(predict[-1, -1, -1])
        # print(predict[-1, -1, -1] * (valueMax[-1] - valueMin[-1]) + valueMin[-1])
        ...
        print(" day %d real : %f , predict %f" % (day, realData, predictPrice))
    dataframe = pd.DataFrame({'real': realValues, 'prediction': predictValues})
    dataframe.to_csv('./loss5/result.csv', index=False, sep=',')
