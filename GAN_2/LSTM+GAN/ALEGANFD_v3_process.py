...
class GANFD():
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as self.sess:
            ...
            if not os.path.isdir('./loss_lstm_GAN/'):
                os.mkdir('./loss_lstm_GAN/')

            if not os.path.isdir('./loss_lstm_GAN/%d/' % day):
                os.mkdir('./loss_lstm_GAN/%d/' % day)

            dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
            dataframe.to_csv('./loss_lstm_GAN/%d/realFakeLoss.csv' % day, index=False, sep=',')

            dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
            dataframe.to_csv('./loss_lstm_GAN/%d/GANLoss.csv' % day, index=False, sep=',')
