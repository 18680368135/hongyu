...
class train_model():
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            ...
            if not os.path.isdir('./loss_BiLSTM/'):
                os.mkdir('./loss_BiLSTM/')

            if not os.path.isdir('./loss_BiLSTM/%d/' % day):
                os.mkdir('./loss_BiLSTM/%d/' % day)


            dataframe = pd.DataFrame({'bilstm': trainHist['squareLoss']})
            dataframe.to_csv('./loss_BiLSTM/%d/Loss.csv' % day, index=False, sep=',')
