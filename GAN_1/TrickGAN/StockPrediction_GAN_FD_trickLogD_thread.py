...
from GANFD_trickLogD import GANFD
import pymysql, os
import numpy as np
import pandas as pd
...
GPU_useage = multiprocessing.Array('i', [3, 3, 1, 1, 1, 1, 1, 1])
...
def release_gpu(gpu, release_lock):
    with release_lock:
        GPU_useage[gpu] += 1
...
def readqueue(q):
    ...
    df.to_csv('./loss_trickLogD/result.csv', index=False, sep=',')
