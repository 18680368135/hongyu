...
real_values = df.iloc[:, 0].values
predict_values = df.iloc[:, 1].values
...
print('MAPE:%.4f' % MAPE)
print('MAE:%.4f' % MAE)
plt.plot(x, real_values, color='blue', label=title[0])
plt.plot(x, predict_values, color='red', label=title[1])
plt.legend()

plt.xlabel('day')
plt.ylabel('value')
plt.savefig('./10daysresult.jpg')
plt.show()
