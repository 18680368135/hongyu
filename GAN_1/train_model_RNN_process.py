...
class train_model():
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            ...
            if not os.path.isdir('./loss_RNN/'):
                os.mkdir('./loss_RNN/')

            if not os.path.isdir('./loss_RNN/%d/' % day):
                os.mkdir('./loss_RNN/%d/' % day)


            dataframe = pd.DataFrame({'rnn': trainHist['squareLoss']})
            dataframe.to_csv('./loss_RNN/%d/Loss.csv' % day, index=False, sep=',')
