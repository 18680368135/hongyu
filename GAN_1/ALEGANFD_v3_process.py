...
class GANFD():
    ...
    def buildModel(self):
        ...
        self.gOptim_ls = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.99).minimize(self.gLoss_ls, var_list=self.GVars)
        self.gOptim_minimax = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.99).minimize(self.gLoss_minimax, var_list=self.GVars)
        self.gOptim_trickLogD = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.99).minimize(self.gLoss_trickLogD, var_list=self.GVars)
    ...
    def trainAndPredict(self, data, dataProcessor, day, parprocess, chiprocess):
        """
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as self.sess:
            with self.graph.as_default():
                ...
                for st_index in range(step_index_len):
                    print('父进程：%d, 子进程：%d, day %d Epoch %d of %d, ' % (parprocess, chiprocess, day, epoch, self.epochs))
                    genIn, real = [], []
                    ...
                    if st_index == 0:
                        ...
                        for can_i in range(0, self.ncandi):
                            ...
                            if frScore[0] < 1e-5 and frScore[1] < 1e-5 and frScore[2] < 1e-5:
                                return _, True
            ...
            if not os.path.isdir('./loss4/'):
                os.mkdir('./loss4/')

            if not os.path.isdir('./loss4/%d/' % day):
                os.mkdir('./loss4/%d/' % day)

            dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
            dataframe.to_csv('./loss4/%d/realFakeLoss.csv' % day, index=False, sep=',')

            dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
            dataframe.to_csv('./loss4/%d/GANLoss.csv' % day, index=False, sep=',')
