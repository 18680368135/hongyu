...
class train_model():
    ...
    def trainAndPredict(self, data, dataProcessor, day):
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            ...
            if not os.path.isdir('./loss_LSTM/'):
                os.mkdir('./loss_LSTM/')

            if not os.path.isdir('./loss_LSTM/%d/' % day):
                os.mkdir('./loss_LSTM/%d/' % day)


            dataframe = pd.DataFrame({'lstm': trainHist['squareLoss']})
            dataframe.to_csv('./loss_LSTM/%d/Loss.csv' % day, index=False, sep=',')
