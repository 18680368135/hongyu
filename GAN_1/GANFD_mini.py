...
class GANFD():
    ...
    def buildModel(self):
        ...
        self.dfake = tf.reduce_mean(self.dLogitsFake)
        ...
        self.gLoss = -tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                ...
                labels=tf.zeros_like(self.dLogitsFake)))  # + self.squareLoss + directLoss
    ...
    def trainAndPredict(self, data, dataProcessor, day, parprocess, chilprocess):
        """
        ...
        with tf.Session(graph=self.graph, config=self.my_config).as_default() as sess:
            with self.graph.as_default():
                ...
                for epoch in range(self.epochs):
                    print('父进程： %d  子进程  %d  day  %d  Epoch %d of %d' %
                          (parprocess, chilprocess, day, epoch + 1, self.epochs))
                    GLosses, DLosses = [], []
                    pReal, pFake = [], []
                    squareLoss = []
                    ...
                    for step in range(len(batchIndex) - 1):
                        ...
                        if (iterations - 1) % self.k == 0:
                            _, gLossVal, pRealVal, pFakeVal, sqrLoss, dlfake= sess.run(
                                [self.gOptim, self.gLoss, self.pReal, self.pGen, self.squareLoss, self.dfake],
                                feed_dict={
                                    self.genInput: genIn,
                                    self.realData: real
                                })
                        ...
                        iterations += 1
                    # print("dloss : %.10f, gloss:%.10f, preal: %.10f, pFake : %.10f, dlfake : %r" %
                    # (dLossVal, gLossVal, pRealVal, pFakeVal, dlfake))
                ...
                if not os.path.isdir('./loss_mini/'):
                    os.mkdir('./loss_mini/')

                if not os.path.isdir('./loss_mini/%d/' % day):
                    os.mkdir('./loss_mini/%d/' % day)

                dataframe = pd.DataFrame({'real': trainHist['pReal'], 'fake': trainHist['pFake']})
                dataframe.to_csv('./loss_mini/%d/realFakeLoss.csv' % day, index=False, sep=',')

                dataframe = pd.DataFrame({'D': trainHist['DLoss'], 'G': trainHist['GLoss']})
                dataframe.to_csv('./loss_mini/%d/GANLoss.csv' % day, index=False, sep=',')
                dataframe = pd.DataFrame({'D': trainHist['squareLoss']})
                dataframe.to_csv('./loss_mini/%d/Loss.csv' % day, index=False, sep=',')
